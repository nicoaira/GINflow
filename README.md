...
5. Build a reusable window database (`--index faiss --faiss_index FlatIP` by default; other FAISS types or `--index scann`) and/or search it for seeds. See [docs/indexes.md](docs/indexes.md). `--faiss_gpu` needs `-profile gpu` and does not apply to ScaNN.
